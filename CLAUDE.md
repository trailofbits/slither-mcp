...
This document provides context and guidance for AI assistants (Claude) working with the Slither MCP Server codebase.
...
- `detector_results`: List of Slither detector findings
...
- `InheritanceTree`: Recursive tree structure for inheritance relationships
...
```bash
# Run server
uv run slither-mcp /path/to/project

# Run with cache
uv run slither-mcp /path/to/project --use-cache
