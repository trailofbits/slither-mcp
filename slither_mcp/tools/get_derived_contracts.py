...
class DerivedNode(BaseModel):
    ...
    contract_key: Annotated[
        ContractKey,
        Field(description="The contract key for this node in the derived tree")
    ]
...
class GetDerivedContractsRequest(BaseModel):
...
def build_derived_tree(
    ...
    for potential_child_key, potential_child_model in project_facts.contracts.items():
        # Check if this potential child directly inherits from our contract
        if contract_key in potential_child_model.directly_inherits:
            ...
            derived_contracts.append(
                build_derived_tree(potential_child_key, project_facts, visited.copy())
            )
    
    return DerivedNode(
        contract_key=contract_key,
        derived_by=derived_contracts
    )
...
def get_derived_contracts(
    ...
    # Build the recursive derived contracts tree
    derived_tree = build_derived_tree(request.contract_key, project_facts)
