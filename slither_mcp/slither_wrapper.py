...
def _find_forge_executable():
    ...
    print(
        "Warning: Could not locate forge executable, trying 'forge' command directly",
        file=sys.stderr,
    )
    return "forge"
...
def _find_npx_executable():
    ...
    print(
        "Warning: Could not locate npx executable, trying 'npx' command directly", file=sys.stderr
    )
    return "npx"
...
class LazySlither:
    ...
    def __init__(self, path: str):
        self.path = path
        self._slither: Slither | None = None
        self._built = False
    ...
    def slither(self) -> Slither:
        ...
        self._ensure_built()
        assert self._slither is not None
