...
class TestDetectorIntegration:
    ...
    def test_reentrancy_detector_finds_vulnerability(self, test_path, temp_solidity_project):
    ...
    def test_low_level_calls_detector(self, test_path, temp_solidity_project):
        """Test that low-level calls detector finds issues."""
        contract_file = write_contract(
            temp_solidity_project, "LowLevel.sol", LOW_LEVEL_CALLS_CONTRACT
        )
    ...
    def test_solc_version_detector(self, test_path, temp_solidity_project):
    ...
    def test_detector_metadata_populated(self, test_path, temp_solidity_project):
    ...
    def test_multiple_detectors_find_issues(self, test_path, temp_solidity_project):
...
"""
        ...
        detectors_with_results = [
            name for name, results in facts.detector_results.items() if results
        ]
        assert len(detectors_with_results) >= 2, (
            f"Expected multiple detectors, got {len(detectors_with_results)}"
        )

    def test_detector_results_have_unique_checks(self, test_path, temp_solidity_project):
    ...
    def test_detector_source_locations_are_valid(self, test_path, temp_solidity_project):
        ...
        for _detector_name, results in facts.detector_results.items():
            if not results:
                continue
...
class TestDetectorIntegrationErrorHandling:
    ...
    def test_invalid_contract_graceful_failure(self, test_path, temp_solidity_project):
...
"""
        ...
        # Should raise an error during Slither initialization (compilation error)
        with pytest.raises((Exception,)):  # noqa: B017 - Slither may raise various errors
            lazy_slither = LazySlither(temp_solidity_project)
            get_project_facts(temp_solidity_project, lazy_slither)

    def test_empty_project_has_no_detector_results(self, test_path, temp_solidity_project):
