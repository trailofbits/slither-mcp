...
class TestSlitherMCPClientConnection:
    ...
    async def test_connect_no_path_argument(self):
        ...
        with patch(
            "slither_mcp.client.mcp_client.stdio_client", return_value=mock_stdio_context
        ) as mock_stdio:
            with patch("slither_mcp.client.mcp_client.ClientSession", return_value=mock_session):
                await client.connect()
...
class TestSlitherMCPClientTools:
    ...
    async def test_list_contracts(self, mock_connected_client):
        ...
        # Mock the response
        expected_response = ListContractsResponse(success=True, contracts=[], total_count=0)
    ...
    async def test_get_contract(self, mock_connected_client, child_contract_key, child_contract):
        ...
        # Mock the response
        expected_response = GetContractResponse(success=True, contract=child_contract)
        ...
        request = GetContractRequest(
            path="/test/project", contract_key=child_contract_key, include_functions=True
        )
    ...
    async def test_list_functions(self, mock_connected_client, child_contract_key):
        ...
        # Mock the response
        expected_response = ListFunctionsResponse(success=True, functions=[], total_count=0)
    ...
    async def test_get_inherited_contracts(self, mock_connected_client, child_contract_key):
        ...
        expected_response = GetInheritedContractsResponse(
            success=True, contract_key=child_contract_key, full_inheritance=None
        )
        ...
        request = GetInheritedContractsRequest(
            path="/test/project", contract_key=child_contract_key
        )
    ...
    async def test_list_function_implementations(self, mock_connected_client, child_contract_key):
        ...
        # Mock the response
        expected_response = ListFunctionImplementationsResponse(success=True, implementations=[])
        ...
        assert len(response.implementations) == 0
        mock_session.call_tool.assert_called_once()
...
class TestSlitherMCPClientHelpers:
    ...
    async def test_get_all_contracts(
        self, mock_connected_client, child_contract_key, child_contract
    ):
        ...
        get_response = GetContractResponse(success=True, contract=child_contract)
    ...
    async def test_get_all_contracts_empty(self, mock_connected_client):
        ...
        # Mock empty response
        list_response = ListContractsResponse(success=True, contracts=[], total_count=0)
    ...
    @pytest.mark.asyncio
    async def test_get_project_facts(
        self, mock_connected_client, child_contract_key, child_contract
    ):
...
class TestSlitherMCPClientErrorHandling:
    ...
    async def test_tool_returns_error_response(self, mock_connected_client):
        ...
        error_response = ListContractsResponse(
            success=False, contracts=[], total_count=0, error_message="Contract not found"
        )
